...
# https://github.com/kurenaif/dotfiles/blob/master/fish_prompt.fish
...
function fish_prompt
  ...
  if [ $status -eq 0 ]
    set status_face (set_color black)"( ^ω^)"
  else
    set status_face (set_color blue)"( ×ω×)"
  end

  echo -n (set_color black)"╭─"(set_color normal)
  #__user_host
  __current_path
  ##__informative_git_prompt
  echo -e ''
  echo (set_color black)"╰─"$status_face(set_color black)" \$ "(set_color normal)
end
